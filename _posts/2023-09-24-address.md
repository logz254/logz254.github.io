---
layout: post
title: Address
date: 2023-09-24 16:15 +0300
categories: [SheHacks Intervarsity CTF, Address]
---
## Question
Find the ip and port called by the malicious script. Flag format: SHCTFf{ip:port}
File: [Book.xls]({% flexible_include file="assets/CTFs-main/SIC(AspireCTF)/Address/books.xls"%})


## Solution
The first thing I realized on seeing that the file extension as .xls was that the malicious script was written in the Macros of the Microsoft Excel document.

<!-- ![Alt text](assets\CTFs-main\SIC(AspireCTF)\Address\opened_document.png) -->
<!-- ({% flexible_include file="assets\CTFs-main\SIC(AspireCTF)\Address\opened_document.png" %}) -->
Click on "Enable Content" so that the Macros are enabled. You receive the prompt as shown below.
![Alt text](assets\CTFs-main\SIC(AspireCTF)\Address\prompt.png)
Click "No".

Select "View" tab then Macros->View Macros->Select Auto_run->Edit
![Alt text](assets\CTFs-main\SIC(AspireCTF)\Address\macro_menu.png)

![Alt text](assets\CTFs-main\SIC(AspireCTF)\Address\macro_code.png)
The part of the malicious script we are interested in is "powershell -e "

This command executes a PowerShell script encoded in Base64 encoding.
To better understand this part of the script, we need to save it to a file. In this case, I used address.txt and then decoded it as shown below.
![Alt text](assets\CTFs-main\SIC(AspireCTF)\Address\base64_decoding.png)

Therefore, we get the IP address and the port.
SHCTF{127.0.0.1:9001}

